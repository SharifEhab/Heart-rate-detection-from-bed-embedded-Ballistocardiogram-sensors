...
def process_bcg_window(bcg_window, time_window, subj, date, window_idx):
    ...
    if len(j_peak_indices) > 1:
        ...
        hr_values_all = 60 / j_peak_intervals
        peak_times_all = j_peak_indices[1:] / TARGET_F_SAMPLE  # Time of each peak after the first one
        # Filter heart rates between 40 and 150 BPM
        mask = (hr_values_all >= 40) & (hr_values_all <= 150)
        hr_values = hr_values_all[mask]
        peak_times = peak_times_all[mask]
    ...
    if len(hr_values) > 0:
        ...
        if hr_avg is not None:
            plt.axhline(y=hr_avg, color='r', linestyle='--', label=f'Average HR: {hr_avg:.1f} BPM')
